...
public class RabbitConfiguration {
    ...
    Binding binding_vnfmCoreAction(TopicExchange exchange) {
        return BindingBuilder.bind(queue_vnfmCoreActions()).to(exchange).with(queueName_vnfmCoreActions);
    }

    @Bean
    Binding binding_vnfmCoreActionReply(TopicExchange exchange) {
        return BindingBuilder.bind(queue_vnfmCoreActionsReply()).to(exchange).with(queueName_vnfmCoreActionsReply);
    }

    @Bean
    Binding binding_eventUnregister(TopicExchange exchange) {
        return BindingBuilder.bind(queue_eventUnregister()).to(exchange).with(queueName_eventUnregister);
    }
    ...
    @Bean
    MessageListenerAdapter listenerAdapter_vnfmRegister(@Qualifier("rabbitRegister") VnfmRegister rabbitRegister) {
        return new MessageListenerAdapter(rabbitRegister, "addManagerEndpoint");
    }

    @Bean
    MessageListenerAdapter listenerAdapter_vnfmUnregister(@Qualifier("rabbitRegister") VnfmRegister rabbitUnregister) {
        return new MessageListenerAdapter(rabbitUnregister, "removeManagerEndpoint");
    }

    @Bean
    MessageListenerAdapter listenerAdapter_vnfmCoreActions(@Qualifier("rabbitVnfmReceiver") VnfmReceiver vnfmReceiver) {
        MessageListenerAdapter actionFinished = new MessageListenerAdapter(vnfmReceiver, "actionFinishedVoid");
        return actionFinished;
    }

    @Bean
    MessageListenerAdapter listenerAdapter_vnfmCoreActionsReply(@Qualifier("rabbitVnfmReceiver") VnfmReceiver vnfmReceiver) {
        MessageListenerAdapter actionFinished = new MessageListenerAdapter(vnfmReceiver, "actionFinished");
        return actionFinished;
    }


    @Bean
    SimpleMessageListenerContainer container_eventRegister(ConnectionFactory connectionFactory, @Qualifier("listenerAdapter_eventRegister") MessageListenerAdapter listenerAdapter) {
    ...
    @Bean
    SimpleMessageListenerContainer container_eventUnregister(ConnectionFactory connectionFactory, @Qualifier("listenerAdapter_eventUnregister") MessageListenerAdapter listenerAdapter) {
        SimpleMessageListenerContainer container = new SimpleMessageListenerContainer();
        container.setConnectionFactory(connectionFactory);
    ...
    @Bean
    SimpleMessageListenerContainer container_vnfmRegister(ConnectionFactory connectionFactory, @Qualifier("listenerAdapter_vnfmRegister") MessageListenerAdapter listenerAdapter) {
        SimpleMessageListenerContainer container = new SimpleMessageListenerContainer();
        container.setConnectionFactory(connectionFactory);
    ...
    @Bean
    SimpleMessageListenerContainer container_vnfmUnregister(ConnectionFactory connectionFactory, @Qualifier("listenerAdapter_vnfmUnregister") MessageListenerAdapter listenerAdapter) {
        SimpleMessageListenerContainer container = new SimpleMessageListenerContainer();
        container.setConnectionFactory(connectionFactory);
    ...
    @Bean
    SimpleMessageListenerContainer container_vnfmCoreActions(ConnectionFactory connectionFactory, @Qualifier("listenerAdapter_vnfmCoreActions") MessageListenerAdapter listenerAdapter) {
        SimpleMessageListenerContainer container = new SimpleMessageListenerContainer();
        container.setConnectionFactory(connectionFactory);
    ...
    @Bean
    SimpleMessageListenerContainer container_vnfmCoreActionsReply(ConnectionFactory connectionFactory, @Qualifier("listenerAdapter_vnfmCoreActionsReply") MessageListenerAdapter listenerAdapter) {
        SimpleMessageListenerContainer container = new SimpleMessageListenerContainer();
        container.setConnectionFactory(connectionFactory);
