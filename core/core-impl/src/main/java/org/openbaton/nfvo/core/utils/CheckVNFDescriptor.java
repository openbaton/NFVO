...
public class CheckVNFDescriptor {
    ...
    public static void checkIntegrity(String vnfdJson) throws DescriptorWrongFormat {
        ...
        try {
            virtualNetworkFunctionDescriptor = mapper.fromJson(vnfdJson, VirtualNetworkFunctionDescriptor.class);
        } catch (Exception e) {
    ...
    private static void checkVNFDIntegrity(VirtualNetworkFunctionDescriptor vnfd)
        ...
        if (vnfd.getType() == null || vnfd.getType().isEmpty()) {
            throw new NetworkServiceIntegrityException(
                    "Not found type of VNFD " + vnfd.getName());
        }
        ...
        if (vnfd.getEndpoint() == null
                || vnfd.getEndpoint().isEmpty()) {
            throw new NetworkServiceIntegrityException(
                    "Not found endpoint in VNFD " + vnfd.getName());
        }
        ...
        if (vnfd.getDeployment_flavour() != null  && !vnfd.getDeployment_flavour().isEmpty()) {
            for (DeploymentFlavour deploymentFlavour :  vnfd.getDeployment_flavour()) {
                if (deploymentFlavour.getFlavour_key() == null  || deploymentFlavour.getFlavour_key().isEmpty()) {
                    throw new NetworkServiceIntegrityException( "Deployment flavor of VNFD "+ vnfd.getName() + " is not well defined");
                }
            }
        } else {
