...
COPY --from=builder /project/etc/openbaton.properties /etc/openbaton/nfvo.properties
ENV NFVO_PLUGIN_INSTALLATION-DIR /usr/lib/openbaton/plugins
ENTRYPOINT ["java", "-jar", "/nfvo.jar", "--spring.config.location=file:/etc/openbaton/nfvo.properties"]
EXPOSE 8080
EXPOSE 8443
