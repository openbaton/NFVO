...
angular.module('app').
    controller('vimInstanceCtrl', function($scope, $routeParams, http, serviceAPI) {
...
//                    "longitude": 11.58198
        ...
        $scope.refreshDc = function() {
            ...
            http.get(url + $routeParams.vim-instanceId + '/refresh')
        ...
        $scope.updateDC = function(datacenter) {
            ...
            http.put(url + datacenter.id, copyDatacanter)
                .success(function(response) {
        ...
        function loadVIM() {
            if (!angular.isUndefined($routeParams.vim-instanceId))
                http.syncGet(url + $routeParams.vim-instanceId).then(function(data) {
                    $scope.datacenter = data;
                    $scope.datacenterJSON = JSON.stringify(data, undefined, 4);
                    $scope.upDatacenter = data;
                });
        ...
        function showError(data, status) {
            ...
            $scope.alerts.push({type: 'danger', msg: 'ERROR: <strong>HTTP status</strong>: ' + status + ' response <strong>data</strong> : ' + JSON.stringify(data)});
