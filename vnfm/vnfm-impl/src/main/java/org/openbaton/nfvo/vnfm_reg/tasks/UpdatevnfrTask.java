...
public class UpdatevnfrTask extends AbstractTask {
  ...
  protected NFVMessage doWork() throws Exception {
    ...
    virtualNetworkFunctionRecord_nfvo.setLocalization(
        virtualNetworkFunctionRecord.getLocalization());
    virtualNetworkFunctionRecord_nfvo.setMonitoring_parameter(
        virtualNetworkFunctionRecord.getMonitoring_parameter());
    virtualNetworkFunctionRecord_nfvo.setNotification(
        virtualNetworkFunctionRecord.getNotification());
    virtualNetworkFunctionRecord_nfvo.setVnf_address(virtualNetworkFunctionRecord.getVnf_address());
    virtualNetworkFunctionRecord_nfvo.setRuntime_policy_info(
        virtualNetworkFunctionRecord.getRuntime_policy_info());
    virtualNetworkFunctionRecord_nfvo.setProvides(
        updateConfiguration(
            virtualNetworkFunctionRecord_nfvo.getProvides(),
            virtualNetworkFunctionRecord.getProvides()));
    virtualNetworkFunctionRecord_nfvo.setRequires(
        updateConfiguration(
            virtualNetworkFunctionRecord_nfvo.getRequires(),
            virtualNetworkFunctionRecord.getRequires()));
    virtualNetworkFunctionRecord_nfvo.setConfigurations(
        updateConfiguration(
    ...
    for (VirtualDeploymentUnit vdu_manager : virtualNetworkFunctionRecord.getVdu()) {
      ...
      for (VirtualDeploymentUnit vdu_nfvo : virtualNetworkFunctionRecord_nfvo.getVdu()) {
        ...
        if (vdu_nfvo.getId().equals(vdu_manager.getId())) {
          ...
          vdu_nfvo.setVirtual_memory_resource_element(
              vdu_manager.getVirtual_memory_resource_element());
    ...
    return new OrVnfmGenericMessage(virtualNetworkFunctionRecord, Action.UPDATEVNFR);
  }

  private Set<VNFComponent> updateVNFComponents(
  ...
  private Set<VNFCInstance> updateVNFCInstances(
    ...
    for (VNFCInstance vnfcInstance_manager : vnfcInstances_manager) {
      ...
      for (VNFCInstance vnfcInstance_nfvo : vnfcInstances_nfvo) {
        ...
        if (vnfcInstance_nfvo.getId().equals(vnfcInstance_manager.getId())) {
          ...
          vnfcInstance_nfvo.setConnection_point(
              updateVNFDConnectionPoints(
  ...
  private Configuration updateConfiguration(
      Configuration configuration_nfvo, Configuration configuration_manager) {
    configuration_nfvo.setName(configuration_manager.getName());
    configuration_nfvo.setConfigurationParameters(
        updateConfigurationParameters(
            configuration_nfvo.getConfigurationParameters(),
            configuration_manager.getConfigurationParameters()));
